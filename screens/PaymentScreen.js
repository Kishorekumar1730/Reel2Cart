...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Alert, ActivityIndicator, StatusBar, Image, Linking } from 'react-native';
...
const PaymentScreen = () => {
    ...
    const createOrder = async (pId) => {
        try {
            ...
            const token = await AsyncStorage.getItem("authToken"); // Or however you called it, usually 'authToken' or 'token' in login
            // Checking LoginScreen.js logic would confirm token key. Assuming 'authToken' or similar. 
            // Wait, previous turns showed LoginScreen: const token = res.token; await AsyncStorage.setItem("authToken", token);
            // Let's verify token key first to be safe. 
            // LoginScreen logic: await AsyncStorage.setItem("authToken", token);
